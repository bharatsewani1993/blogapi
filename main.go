...
func dbconnection() *sql.DB {
	db, err := sql.Open("mysql", "root:123456789@/golangtask")
	if err != nil {
		fmt.Printf("Error %v\n", err)
	}
	return db
...
func loadpage(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
    db := dbconnection()
		...
		pageid := ps.ByName("id")
		s, err := strconv.ParseInt(pageid, 10, 64)
		pageid = s * 10
...
func postarticle(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
		...
		if len(r.Form.Get("nickname")) !=0 && len(r.Form["title"][0])!=0 && len(r.Form["content"][0])!=0{
...
func postcomment(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
	...
	if len(r.Form.Get("nickname")) !=0 && len(r.Form["article_id"][0])!=0 && len(r.Form["content"][0])!=0{
		db := dbconnection()
		currenttime := time.Now().Local()
		ct := currenttime.Format("2006-01-02")
...
func postsubcomment(w http.ResponseWriter, r *http.Request, ps httprouter.Params){
	...
	if len(r.Form.Get("nickname")) !=0 && len(r.Form["comment_id"][0])!=0 && len(r.Form["content"][0])!=0 {
	db := dbconnection()
	currenttime := time.Now().Local()
	ct := currenttime.Format("2006-01-02")
